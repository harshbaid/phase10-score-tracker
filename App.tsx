...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-50 pb-20 transition-colors duration-200">
      {/* Header */}
      <header className="bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 sticky top-0 z-30 transition-colors duration-200">
        <div className="max-w-4xl mx-auto px-4 py-4 flex justify-between items-center">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-indigo-600 rounded-lg flex items-center justify-center text-white shadow-lg shadow-indigo-200 dark:shadow-indigo-900/50">
               <span className="font-black text-xl">10</span>
            </div>
            <h1 className="text-xl font-extrabold tracking-tight text-gray-800 dark:text-white">Phase 10 <span className="text-indigo-600 dark:text-indigo-400">Tracker</span></h1>
          </div>
          
          <div className="flex gap-2 items-center">
            <ThemeToggle />
            {gameState.gameStartedAt > 0 && (
              <Button variant="ghost" onClick={resetGame} className="text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        {gameState.isGameOver && (
          <div className="mb-8 bg-gradient-to-r from-indigo-600 to-blue-500 dark:from-indigo-800 dark:to-blue-700 rounded-2xl p-8 text-white text-center shadow-xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          ...
          {!gameState.gameStartedAt && (
            <div className="flex flex-col gap-3">
              {isAddingPlayer ? (
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-indigo-200 dark:border-indigo-500/30 p-4 flex flex-col gap-3 transition-colors duration-200">
                  <input 
                    autoFocus
                    type="text"
                    placeholder="Player Name"
                    className="w-full px-4 py-2 bg-white dark:bg-slate-900 text-gray-900 dark:text-white border border-gray-200 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-colors duration-200 placeholder-gray-400 dark:placeholder-gray-500"
                    value={newPlayerName}
                ...
                <button 
                  ...
                  className="h-full min-h-[140px] border-2 border-dashed border-gray-300 dark:border-slate-700 rounded-xl flex flex-col items-center justify-center text-gray-400 dark:text-gray-500 hover:text-indigo-500 dark:hover:text-indigo-400 hover:border-indigo-400 dark:hover:border-indigo-500 hover:bg-indigo-50/30 dark:hover:bg-indigo-900/10 transition-all group"
                >
          ...
          <section className="mt-12">
            ...
            <div className="space-y-4">
              {gameState.history.map((round, idx) => (
                <div key={round.id} className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden transition-colors duration-200">
                  <div className="bg-gray-50 dark:bg-slate-800/50 px-4 py-2 border-b border-gray-100 dark:border-slate-700 flex justify-between items-center">
                    <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Round {gameState.history.length - idx}</span>
                    <div className="flex items-center gap-4">
                      <span className="text-xs text-gray-400 dark:text-gray-500">{new Date(round.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      <button
                        onClick={() => editRound(round.id)}
                        className="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 uppercase tracking-wider transition-colors flex items-center gap-1"
                      >
                        ...
                        <div key={entry.playerId} className="px-4 py-3 flex justify-between items-center">
                          ...
                          <div className="flex items-center gap-4">
                            {entry.phaseCompleted && (
                              <span className="text-[10px] bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 font-bold px-2 py-0.5 rounded-full uppercase">Phase Up!</span>
                            )}
                            <span className="font-black text-gray-800 dark:text-white">+{entry.points}</span>
                          </div>
                        </div>
                      );
                    })}
